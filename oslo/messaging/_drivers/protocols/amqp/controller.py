...
class Controller(engine.ConnectionEventHandler):
    ...
    def connection_closed(self, connection):
        if self._closing:
            LOG.debug("Close confirmed, closing socket and shutting down")
            self._complete_shutdown()

    def connection_remote_closed(self, connection, reason):
        if self._closing:
            LOG.debug("Closed by peer, closing socket and shutting down")
            self._complete_shutdown()
        elif reason:
    ...
    def _put_task(self, task):
        ...
        if self._replies and self._replies.ready() and not self._closing:
            self.processor.wakeup(lambda: self._process_tasks())
