...
class ProtonIncomingMessage(base.IncomingMessage):
    ...
    def reply(self, reply=None, failure=None, log_failure=True):
        if self._reply_to:
            ...
            LOG.warn("Ignoring reply as no reply address available")
...
class ProtonDriver(base.BaseDriver):
    ...
    def listen_for_notifications(self, targets_and_priorities):
        ...
        for target, priority in targets_and_priorities:
            target.topic = '%s.%s' % (target.topic, priority)
            self._mgr.tasks().put(ListenTask(target, listener._incoming, True))
        return listener
